...
data_arg.add_argument('--max_length', type=int, default=20, help='number of cities') ##### #####
data_arg.add_argument('--dimension', type=int, default=2, help='city dimension')
data_arg.add_argument('--num_perms', type=int, default=5, help='number of heuristic perms for avg')
...
class Actor(object):
    ...
    def __init__(self):
        ...
        self.predictions = tf.placeholder(tf.float32, [None], name="input_predictions")
...
with tf.Session() as sess: # start session
    ...
    heuristic_predictions = np.zeros(config.batch_size)
    ...
    for i in tqdm(range(config.nb_steps)): # Forward pass & train step
        ...
        for j in range(actor.batch_size):
            temp_reward_array = []
            for k in range(actor.num_perms):
                perm_idx = np.random.permutation(actor.max_length)
                _, temp_reward = dataset.loop2opt(input_batch[j][perm_idx])
                temp_reward_array.append(temp_reward)
            heuristic_predictions[j] = np.mean(temp_reward_array)
        feed = {actor.input_: input_batch, actor.predictions:heuristic_predictions} # get feed dict

        reward, predictions, summary, _ = sess.run([actor.reward, actor.predictions, actor.merged, actor.trn_op1], feed_dict=feed)
